...
%               For other values of n, the threshold are found by
...
%
%
%       robest: robust estimator. String. String that identifies the robust estimator which is used
%               Possible values are:
%                 'S'    S estimators;
%                 'MM'   MM estimators;
...
%               threshold found for bdp=0.25 is used. In this case, a
...
%               the asymptotic consistency factor using the breakdown point
...
%               simple average of the thresholds associated to the two
...
%                Empirical threshold which can be used in order
%                to have a test with an empirical size close to the nominal
...
%               250, 300, 400, 500.
%               The 9 columns are referred to the number of variables which
%               have been considered namely p=2, 3, ..., 10.
%               The third dimension is associated with the 24 estimators
%               that have been used. The order of the estimators is:
%                 ' 1'    'LTSbdp050' ;
%                 ' 2'    'LTSbdp025' ;
%                 ' 3'    'LTSrbdp050';
%                 ' 4'    'LTSrbdp025';
...
%{
    ...
    % Find the threshold for MM estimator, take an average threshold for all
...
if nargin <3 || isempty(robest)
    ...
    % check if the estimator, which has been provided is in the list.
...
% Check type of robust estimator and rho function. 
...
if strcmp(robest,'MM')==1
    ...
    if intersect(effToUse,effchk)==effToUse
        ...
        % average of the thresholds for all the weight function for a given
        % value of efficiency.
        % posint is the boolean vector that contains the position
        ...
        if strcmp(rhofunc,'ST')
            ...
            % If the weight function is not equal to ST, then it is
    ...
    elseif  effToUse<0.85
        % If the user wants to have an efficiency smaller than 0.85, then the
    ...
    elseif  effToUse>0.85 && effToUse <0.9
        if strcmp(rhofunc,'ST')
            ...
            % posint = logical vector with true for the indexes in the
            % position associated with the MM estimators that have 0.85 or
            % 0.90 efficiency.
    ...
    elseif  effToUse>0.90 && effToUse <0.95
        ...
        % If the user wants to have an efficiency greater than 0.95, then the
...
elseif strcmp(robest,'S')==1
    ...
    if intersect(bdp,bdpchk)==bdp
        ...
        % average of the thresholds for S estimators associated to all the
...
% Select the slices of third dimension of mat file, which have to be used.
posint1=0;
posint2=0;
...
THsel=[nn' THsel];
% Check if the supplied value of n is in our grid.
seln=find(nn>n);
if isempty(seln) % in this case, n is equal to 500.
    THseln=THsel([size(THsel,1) size(THsel,1)],:);
else
    % Find the two values of n among those of the simulations that enclose
...
% Finally interpolate to obtain the threshold for the value of n requested
% by the user.
nU=yhat(2,1);
nL=yhat(1,1);
